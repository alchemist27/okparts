...
const FIXED_VALUES = {
  sellingPrice: 0,
  categoryNo: 192,
  summaryDescription: "네바퀴 JK모터스",
  sellerPhone: "010-7125-5474",
  description: `부품문의 : 010 - 7125 - 5474
국산차, 수입차 전차종 중고부품 내장, 외장, 하체부품상담

BMW, 벤츠, 아우디, 폭스바겐, 렉서스, 혼다, 링컨, 크라이슬러, 닛산, 도요타, 볼보, 랜드로버, 인피니티, 캐딜락, 포드, 지프, 재규어, 푸조, 현대, 기아, GM대우, 쌍용, 르로삼성, 쉐보레

장안동 장한평역 중고부품 네바퀴

더 많은 정보 클릭 : http://cafe.daum.net/xodid2011

수입차 백미러 수리 & 라이트 복원(청소+UV코팅)

라이트, 백미러 수리, 사이드미러 수리, 범퍼, 헤드램프, 그릴, 세루커버, 보닛, 본넷, 후드, 문짝, 도어, 레일, 쇼바, 너클, 노아다이, 로웜, 맴바, 등속, 데루등, 테일램프, 트렁크, 휀더, 펜더, 라지에다, 콘덴셔, 휀, 후앙,라디에이터, 콘덴서, 유리, 백미러 수리, 스테프, 휠, 타이어, 장안평, 장한평

중고차부품, 벤츠부품, BMW부품, 폐차부품, 폐차장중고부품, 폐차장부품, 중고자동차부품, 중고부품, 중고범퍼, 자동차 중고부품 각종 자동차부품 취급`,
} as const;
 ...
 *
 * Body: BulkProductRegistrationRequest (JSON)
 *
 * 고정 값:
 * - sellingPrice: 0원
 * - categoryNo: 192
 * - summaryDescription: 부품문의 : 010 - 7125 - 5474
 * - sellerPhone: 010-7125-5474
 * - description: 장안동 장한평역 중고부품 네바퀴
 */
...
async function registerSingleProduct(
  ...
  try {
    // 고정 값 적용
    const finalSellingPrice = FIXED_VALUES.sellingPrice;
    ...
    console.log(`[Bulk Product Create] [${index}] 고정 값 적용: 판매가=${finalSellingPrice}원, 카테고리=${finalCategoryNo}, 요약=${finalSummaryDescription}, 전화번호=${finalSellerPhone}`);
    ...
    const productDoc = await addDoc(productsRef, {
      ...
      supplyPrice: finalSellingPrice, // 고정 값 사용
      sellingPrice: finalSellingPrice, // 고정 값 사용
    ...
    const cafe24ProductData: any = {
      ...
      price: finalSellingPrice, // 고정 값 사용
      supply_price: finalSellingPrice, // 고정 값 사용
      display: product.display || "T",
      selling: product.selling || "T",
